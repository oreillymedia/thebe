define [
  ...
  class Thebe
    ...
    # (but sets up call_spawn on click, for now)
    start_terminal: =>
      $(@selector).one 'click', (e)=>
        if @options.tmpnb_mode
          ...
          # @log @url
          # @log 'Spawn called, start term'

    # equivalent to @start_kernel 
    # i.e. actually starts terminal on the server (after call spawn)
    start_terminal_backend: =>
      # @call_spawn
      console.log @url
      # do
      # POST TO @url+ api/terminals
      ...
      # do we even need that post?
      # yes, turns out we do, do it manually, get the name (here, it's "1")
      # this happens automatically in kernel.js for kernels, but is generated by the backend in normal jupyter times
      # @url = "http://fake.com"
    terminal_start_handler: (e)->
      ...
      name = res["name"]
      ws_url = @url.replace('http', 'ws')+"terminals/websocket/#{name}"
      # ws_url = @url.replace('http', 'ws')+"terminals/#{name}"
      # ws_url = "wss://192.168.99.100:8000/user/O21lXdbPZHjm/terminals/websocket/2"
      # ws_url = "wss://192.168.99.100:8000/user/UIzJMZTIA29L/terminals/websocket/1"
      #           wss://192.168.99.100:8000/user/O21lXdbPZHjm/terminals/websocket/2
      console.log "ws_url:"
      console.log ws_url
